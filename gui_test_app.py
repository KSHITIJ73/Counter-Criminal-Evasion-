...
APP_TITLE = "C.C.E: Counter Criminal Evasion System (UI TEST MODE)"
BG_COLOR = "#212121"
TEXT_COLOR = "#EAEAEA"
...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        # --- Load Face Encodings (Gracefully handle if not found) ---
        self.known_data = self.load_encodings("encodings.pickle")
        self.criminal_list = {"arjun"} # Example criminal list, won't be used if no encodings

        # --- UI Setup ---
        self.create_widgets()
        
    def load_encodings(self, path):
        """Loads face encodings from a pickle file. Returns None if not found."""
        try:
            with open(path, "rb") as f:
                ...
                if "encodings" in data and "names" in data:
                    ...
                    print("[WARNING] encodings.pickle file is corrupt or empty.")
                    messagebox.showwarning("Data File Issue", "The encodings.pickle file was found but seems to be corrupt. Recognition is disabled.")
                    return None
        except FileNotFoundError:
            print("[WARNING] Encoding file not found. Recognition will be disabled.")
            messagebox.showinfo("Info", "encodings.pickle not found. The app will run in detection-only mode.")
            return None
    ...
    def stop_system(self):
        ...
        self.video_label.config(image='', text="System Offline")
    ...
    def process_frame(self, frame):
        ...
        If no encodings are loaded, it will only perform detection.
        """
        ...
        if self.known_data:
            ...
            detected_names = ["Unknown"] * len(face_locations)
